...
import { DATA_SOURCE, WEBRTC_MAX_RECONNECT_RETRIES } from "@/lib/constants";
...
export default function VisionPage() {
  ...
  const { state, stream, stats, reconnectAttempt, start, stop } = useWebRTCStream();
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {reconnectAttempt > 0 && state !== "connected" && (
              <span className="text-xs text-amber-400">
                Reconnecting ({reconnectAttempt}/{WEBRTC_MAX_RECONNECT_RETRIES})
              </span>
            )}
          </div>
          <div className="flex items-center gap-3">
            <CameraSelector
              ...
              disabled={isMock || state === "signaling" || state === "connecting"}
            />
            {isMock ? (
              <span className="text-xs text-zinc-500">
                Set NEXT_PUBLIC_DATA_SOURCE=live to enable streaming
              </span>
            ) : (
              <button
                type="button"
